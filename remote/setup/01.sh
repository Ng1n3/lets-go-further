...
# Install dependencies for secure key management
sudo apt install -y debian-archive-keyring curl gpg

# Download and install Caddy's key properly
curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | \
  sudo gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg

# Add Caddy's repository using the new key
